...
bool DDisplay_IncrementValue(bool wrapAround) {
    ...
    switch(_mode) {
        case DDISPLAY_MODE_DECIMAL:
            ...
            else if(_value == 99 && wrapAround) { // Wrap-around (como carrusel)
                _value = 0;
                result = TRUE;
            }
...
bool DDisplay_DecrementValue(bool wrapAround) {
    bool result = FALSE;
    switch(_mode) {
        case DDISPLAY_MODE_DECIMAL:
            if(_value > 0) {
            ...
            else if(_value == 0 && wrapAround) { // Wrap-around
                _value = 99;
                result = TRUE;
            }
            DDisplay_SetValue(_value);
...
bool DDisplay_NextPattern(bool wrapAround) {
    bool result = FALSE;
    switch(_mode) {
        case DDISPLAY_MODE_PATTERNS:
            if(_patterns != NULL && _patterns_size > 0) {
                
                if(_patterns_index >= _patterns_size - 1) { // Wrap-around
                    // Solo avanzar si está permitido dar la vuelta en el indice final
                    if(wrapAround) {
                        _patterns_index = 0;
                        result = TRUE;
                    }
                // Si no está permitido, result se queda en FALSE y no hacemos nada
                } else {
                    _patterns_index++;
                    result = TRUE;
                }
                
                // Solo si el índice cambió, se actualiza el display
                if(result) {
                    DDisplay_ShowPattern(_patterns_index);
                }
...
bool DDisplay_PreviousPattern(bool wrapAround) {
    bool result = FALSE;
    switch(_mode) {
        case DDISPLAY_MODE_PATTERNS:
            if(_patterns != NULL && _patterns_size > 0) {
    
                // Comprueba si estamos en el primer patrón
                if(_patterns_index == 0) { // Wrap-around
                    if(wrapAround) {
                        _patterns_index = _patterns_index - 1;
                        result = TRUE;
                    }
                    // Si no, result es false
                } else {
                    _patterns_index--;
                    result = TRUE;
                }
